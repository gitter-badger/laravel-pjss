...
class EloquentHistoryRepository implements HistoryContract
{
    ...
    public function renderEntity($entity_id)
    {
        ...
        if (! $history->count())
            return trans("history.backend.none_for_entity", [
                'entity' => $history->type->name
            ]);
    ...
    {
        return '<li>' . '<i class="fa fa-' . $history->icon . ' ' . $history->class . '"></i>' . 

        '<div class="timeline-item">' . '<span class="time"><i class="fa fa-clock-o"></i> ' . $history->created_at->diffForHumans() . '</span>' . '<h3 class="timeline-header no-border"><strong>' . $history->user->name . '</strong> ' . $this->renderDescription($history->text, $history->assets) . '</h3>' . '</div>' . '</li>';
    }
}